...
multi_dittoPlot(c("nCount_RNA", "nFeature_RNA", "percent.mito", "percent.ribo"),
...
```{r}
HSPCs.cut <- importDemux2Seurat(HSPCs.cut,
                                Demuxlet.best = "HSPCs/Demuxlet/HSPC.best")
demux.SNP.summary(object = HSPCs.cut, add.line = 50)
```
...
HSPCs.cut@meta.data$Sample.in.lane <- 
  meta("Sample","HSPCs.cut") %in% c("CD4_APB1803","CD4_APB1804",
                                    "CD4_FS1849", "CD4_FS1850", "CD4_FS1851",
                                    "CD4_SLVR007", "CD4_SLVR063")
...
```{r}
HSPCs.cut@meta.data$Sample <- factor(meta("Sample","HSPCs.cut"),
                                     levels = c("CD4_APB1803","CD4_APB1804",
                                                "CD4_FS1849","CD4_FS1850","CD4_FS1851",
                                                "CD4_SLVR007", "CD4_SLVR063"),
                                     labels = c("APB1803","APB1804",
                                                "FBM1849","FBM1850","FBM1851",
                                                "SLVR007", "SLVR063"))
dittoScatterPlot(x.var = "nCount_RNA", y.var = "nFeature_RNA",
                 object = "HSPCs", cells.use = colnames(HSPCs.cut),
                 show.others = TRUE)
```
...
HSPCs@meta.data$age[grep("FBM",meta("Sample"))] <- "fetal"
HSPCs@meta.data$age[grep("SLVR",meta("Sample"))] <- "cord"
HSPCs@meta.data$age[grep("APB",meta("Sample"))] <- "adult"
```
...
```{r}
# Plot
dittoDimPlot("age", HSPC.recombined, reduction.use = "umap")
dittoDimPlot("integrated_snn_res.1", HSPC.recombined, reduction.use = "umap")
...
dittoDimPlot("blueprint", reduction.use = "umap", do.letter = FALSE,
...
dittoDimPlot("integrated_snn_res.1", reduction.use = "umap",
             main = "Clustering",
             legend.show = FALSE,
...
HSPCs@meta.data$blueprint_hscmpp <- gsub("HSC|MPP", "HSC/MPP", meta("blueprint"))
# Obtain the max call for each cluster
get.max.annot <- function(index){
  calls.table <- table(meta("integrated_snn_res.1", "HSPCs"), meta("blueprint_hscmpp", "HSPCs"))
  colnames(calls.table)[calls.table[index,]==max(calls.table[index,])]
}
HSPCs@meta.data$cluster_max <- 
  factor(meta("integrated_snn_res.1"),
         levels = meta.levels("integrated_snn_res.1"),
         labels = sapply(meta.levels("integrated_snn_res.1"),
...
HSPCs@meta.data$trajectory_calls <- 
  factor(meta("integrated_snn_res.1"),
         meta.levels("integrated_snn_res.1"),
         labels = c("HSC/MPP", # 0
                    "mid", # 1
                    "mid", # 10
                    "CLP", # 11
...
dittoDimPlot("trajectory_calls", reduction.use = "umap",
             ...
             trajectory.cluster.meta = "integrated_snn_res.1",
             trajectory.arrow.size = 0.075,
             cells.use = meta("trajectory_calls")!= "mid",
             colors = c(7,5,6,4)
             )
dev.off()
```
...
renames <- meta.levels("trajectory_calls")[c(5, 3, 4, 1, 2)]
renames[1] <- "N/A"

### Number of cell types per sample:
pdf("HSPCs-Figs/HSPCs_trajectory_calls_bySample.pdf", w=4, h=2.5)
dittoBarPlot("trajectory_calls", group.by = "Sample", main = NULL,
...
dittoBarPlot("age", group.by = "trajectory_calls", main = NULL,
...
all <- table(meta("age"), meta("trajectory_calls"))
FvC <- all[2:3,]
FvA <- all[c(1,3),]
CvA <- all[1:2,]
...
AGE.MPP <- meta("age")
AGE.MPP[meta("trajectory_calls")!="HSC/MPP"] <- 0
Idents(HSPCs) <- AGE.MPP
# dittoDimPlot(cells.use = Idents(HSPCs)!=0)

### 1. Pick half equalized training group based around 30%
Ad.percent.of.FeAd <- sum(meta("ident")=="adult")/sum(meta("ident")%in%c("fetal", "adult"))
...
AGE.MPP.inTrain <- meta("age")
AGE.MPP.inTrain[-inTraining] <- 0
# table(AGE.MPP.inTrain, inTraining, meta("trajectory_calls")
...
FvA.MPP <- FindMarkers(HSPCs,
                       ...
                       ident.2 = "adult",
                       test.use = "MAST")
FvA.MPP_padjFC <- FvA.MPP[abs(FvA.MPP$avg_logFC)>=(log2(1.5)) &
...
nonTrain_FetalAdult_HSCMPP <- !(meta("MPP.inTraining")) & meta("age")!="cord" &
  meta("trajectory_calls")=="HSC/MPP"
roc_obj <- roc(response = 
                 as.numeric(meta("age")=="adult")[nonTrain_FetalAdult_HSCMPP],
               predictor = meta("MPP.RFScore")[nonTrain_FetalAdult_HSCMPP],
               plot = T)
auc(roc_obj)

### Checking plots
# # In training cells
# dittoPlot("MPP.RFScore", cells.use = meta("trajectory_calls")=="HSC/MPP" & meta("MPP.inTraining"),
...
#        vlnplot.lineweight = 0.3, vlnplot.width = 3,
#        sub = "NOT in training")
# dittoDimPlot("MPP.RFScore", cells.use = meta("trajectory_calls")=="HSC/MPP" & !(meta("MPP.inTraining")),
#              sub = "NOT in training", size = 2)

### Main plot output
pdf("HSPCs-Figs/HSPCs_Age-score_HscMpp.pdf", w=3.5, h=2.5)
dittoPlot("MPP.RFScore", group.by = "Sample", color.by = "age",
       cells.use = meta("trajectory_calls")=="HSC/MPP" &
                   !meta("MPP.inTraining"),
...
AGE.MEP <- meta("age")
AGE.MEP[meta("trajectory_calls")!="MEP"] <- 0
...
FvA.MEP <- FindMarkers(HSPCs,
                       ...
                       ident.2 = "adult",
                       test.use = "MAST")
FvA.MEP_padjFC <- FvA.MEP[abs(FvA.MEP$avg_logFC)>=(log2(1.5)) &
...
### 6. Score FvA accuracy
nonTrain_FetalAdult_MEP <- !(meta("MEP.inTraining")) & meta("age")!="cord" &
  meta("trajectory_calls")=="MEP"
roc_obj <- roc(response = 
                 as.numeric(meta("age")=="adult")[nonTrain_FetalAdult_MEP],
               predictor = meta("MPP.RFScore")[nonTrain_FetalAdult_MEP],
               plot = T)
auc(roc_obj)

### Check plots
# # In training cells
# dittoPlot("MEP.RFScore", cells.use = meta("trajectory_calls")=="MEP" & meta("MEP.inTraining"),
#        group.by = "Sample", color.by = "age",
#        plots = c("vlnplot","jitter","boxplot"),
#        boxplot.color = "white", boxplot.fill = F,
...
#        vlnplot.lineweight = 0.3, vlnplot.width = 2,
#        sub = "NOT in training")
# dittoDimPlot("MEP.RFScore", cells.use = meta("trajectory_calls")=="MEP" & !(meta("MEP.inTraining")),
#              sub = "NOT in training", size = 2)

### Main plot output
pdf("HSPCs-Figs/HSPCs_Age-score_MEP.pdf", w=3.5, h=2.5)
dittoPlot("MEP.RFScore", group.by = "Sample", color.by = "age",
              cells.use = meta("trajectory_calls")=="MEP" &
                   !meta("MEP.inTraining"),
       plots = c("jitter", "vlnplot", "boxplot"),
       x.reorder = c(3:5,6,7,1,2),
       boxplot.color = "white", boxplot.fill = FALSE, boxplot.width = 0.075,
...
AGE.GMP <- meta("age")
AGE.GMP[meta("trajectory_calls")!="GMP"] <- 0
...
FvA.GMP <- FindMarkers(HSPCs,
                       ...
                       ident.2 = "adult",
                       test.use = "MAST")
FvA.GMP_padjFC <- FvA.GMP[abs(FvA.GMP$avg_logFC)>=(log2(1.5)) &
...
### 6. Score FvA accuracy
nonTrain_FetalAdult_GMP <- !(meta("GMP.inTraining")) & meta("age")!="cord" &
  meta("trajectory_calls")=="GMP"
roc_obj <- roc(response = 
                 as.numeric(meta("age")=="adult")[nonTrain_FetalAdult_GMP],
               predictor = meta("MPP.RFScore")[nonTrain_FetalAdult_GMP],
               plot = T)
auc(roc_obj)

### Check plots
# # In training cells
# dittoPlot("GMP.RFScore", cells.use = meta("trajectory_calls")=="GMP" & meta("GMP.inTraining"),
#        group.by = "Sample", color.by = "age",
#        plots = c("vlnplot","jitter","boxplot"),
#        boxplot.color = "white", boxplot.fill = F,
#        vlnplot.lineweight = 0.3, vlnplot.width = 2,
...
#        vlnplot.lineweight = 0.3, vlnplot.width = 2,
#        sub = "NOT in training")
# dittoDimPlot("GMP.RFScore", cells.use = meta("trajectory_calls")=="GMP" & !(meta("GMP.inTraining")),
#              sub = "NOT in training", size = 2)

### Main plot output
pdf("HSPCs-Figs/HSPCs_Age-score_GMP.pdf", w=3.5, h=2.5)
dittoPlot("GMP.RFScore", group.by = "Sample", color.by = "age",
              cells.use = meta("trajectory_calls")=="GMP" &
                   !meta("GMP.inTraining"),
       plots = c("jitter", "vlnplot", "boxplot"),
       x.reorder = c(3:5,6,7,1,2),
       boxplot.color = "white", boxplot.fill = FALSE, boxplot.width = 0.075,
...
### Statistical analysis of Immunological Age Scores

```{r}
# HSC/MPP
wilcox.test(meta("MPP.RFScore")[meta("age")=="fetal" & !meta("MPP.inTraining") & meta("trajectory_calls")=="HSC/MPP"],
            meta("MPP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="HSC/MPP"])
wilcox.test(meta("MPP.RFScore")[meta("age")=="adult" & !meta("MPP.inTraining") & meta("trajectory_calls")=="HSC/MPP"],
            meta("MPP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="HSC/MPP"])
```

```{r}
# MEP
wilcox.test(meta("MEP.RFScore")[meta("age")=="fetal" & !meta("MEP.inTraining") & meta("trajectory_calls")=="MEP"],
            meta("MEP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="MEP"])
wilcox.test(meta("MEP.RFScore")[meta("age")=="adult" & !meta("MEP.inTraining") & meta("trajectory_calls")=="MEP"],
            meta("MEP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="MEP"])
```

```{r}
# GMP
wilcox.test(meta("GMP.RFScore")[meta("age")=="fetal" & !meta("GMP.inTraining") & meta("trajectory_calls")=="GMP"],
            meta("GMP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="GMP"])
wilcox.test(meta("GMP.RFScore")[meta("age")=="adult" & !meta("GMP.inTraining") & meta("trajectory_calls")=="GMP"],
            meta("GMP.RFScore")[meta("age")=="cord" & meta("trajectory_calls")=="GMP"])
```
