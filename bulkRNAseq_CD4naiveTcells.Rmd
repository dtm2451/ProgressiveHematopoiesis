...
```{r, echo = T}
bulkCD4 <- importDESeq2(dds, blind = FALSE)
DEFAULT <- "bulkCD4"
```
...
```{r}
# Grab all genes
genes_use <- get.genes(bulkCD4)
# Grab the counts that are non-zero
counts <- bulkCD4@counts
counts_non_zero <- counts>0

# Filter to genes in most samples of each age
in_fetal <- sapply(1:nrow(counts), function(X) sum(counts_non_zero[X,meta("Age")=="Fetal"])>=4)
in_cord <- sapply(1:nrow(counts), function(X) sum(counts_non_zero[X,meta("Age")=="Cord"])>=4)
in_adult <- sapply(1:nrow(counts), function(X) sum(counts_non_zero[X,meta("Age")=="Adult"])>=4)
...
bulkCD4 <- addPrcomp(bulkCD4, prcomp = prcomp(t(bulkCD4@data[genes_use,]), center = TRUE, scale = TRUE))
dittoDimPlot("Age", bulkCD4, reduction.use = "pca")
```

## 6) Extract the Differential Expression results

```{r}
FvA <- as.data.frame(results(bulkCD4@DEobject, contrast=c("Age","Fetal","Adult")))
FvC <- as.data.frame(results(bulkCD4@DEobject, contrast=c("Age","Fetal","Cord")))
CvA <- as.data.frame(results(bulkCD4@DEobject, contrast=c("Age","Cord","Adult")))
...
bulkCD4@meta.data$age.names <- as.factor(as.character(meta("Samples")))
levels(bulkCD4@meta.data$age.names) <- c(paste0("Adult-",1:5),paste0("Fetal-",1:5),paste0("UCB-",1:5))

#Now make the heatmap with DBHeatmap
hm <- dittoHeatmap(genes_use, object = bulkCD4, cell.names.meta = "age.names", show.rownames = FALSE, annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
...
# dittoHeatmap(genes_use, cell.names.meta = "age.names", highlight.genes = names(clust[clust==1]), annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
# dittoHeatmap(genes_use, cell.names.meta = "age.names", highlight.genes = names(clust[clust==2]), annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
# dittoHeatmap(genes_use, cell.names.meta = "age.names", highlight.genes = names(clust[clust==3]), annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
# dittoHeatmap(genes_use, cell.names.meta = "age.names", highlight.genes = names(clust[clust==4]), annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
# dittoHeatmap(genes_use, cell.names.meta = "age.names", highlight.genes = names(clust[clust==5]), annotation.metas = "Age", cutree_rows = 5, cutree_cols = 3, treeheight_col = 10)
...
c2 <- dittoPlotVarsAcrossGroups(names(clust[clust==2]), group.by = "Age", plots = "vlnplot", theme = theme_bw(), y.breaks = c(-1,0,1), x.labels = c("F","U","A"), x.reorder = c(3,2,1), x.labels.rotate = F, main = "Cluster 1", legend.show = F, ylab = NULL, xlab = NULL, sub = paste0(length(clust[clust==2]), " genes"), vlnplot.lineweight = 0.5)
c3 <- dittoPlotVarsAcrossGroups(names(clust[clust==3]), group.by = "Age", plots = "vlnplot", theme = theme_bw(), y.breaks = c(-1,0,1), x.labels = c("F","U","A"), x.reorder = c(3,2,1), x.labels.rotate = F, main = "Cluster 2", legend.show = F, ylab = NULL, xlab = NULL, sub = paste0(length(clust[clust==3]), " genes"), vlnplot.lineweight = 0.5)
c4 <- dittoPlotVarsAcrossGroups(names(clust[clust==4]), group.by = "Age", plots = "vlnplot", theme = theme_bw(), y.breaks = c(-1,0,1), x.labels = c("F","U","A"), x.reorder = c(3,2,1), x.labels.rotate = F, main = "Cluster 3", legend.show = F, ylab = NULL, xlab = NULL, sub = paste0(length(clust[clust==4]), " genes"), vlnplot.lineweight = 0.5)
c5 <- dittoPlotVarsAcrossGroups(names(clust[clust==5]), group.by = "Age", plots = "vlnplot", theme = theme_bw(), y.breaks = c(-1,0,1), x.labels = c("F","U","A"), x.reorder = c(3,2,1), x.labels.rotate = F, main = "Cluster 5", legend.show = F, ylab = NULL, xlab = NULL, sub = paste0(length(clust[clust==5]), " genes"),vlnplot.lineweight = 0.5)

gridExtra::grid.arrange(grobs=list(hm[[4]], c2, c3, c4, c1, c5),
             ncol = 8,
             nrow = 5,
...
c1_genes <- names(clust[clust==2])
c2_genes <- names(clust[clust==3])
c3_genes <- names(clust[clust==4])
c4_genes <- names(clust[clust==1])
c5_genes <- names(clust[clust==5])
...
### All clusters separately

```{r}
#Run pathway enrichment:
c1_ <- enrichKEGG(to.uniprot(c1_genes), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")
c2_ <- enrichKEGG(to.uniprot(c2_genes), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")
c3_ <- enrichKEGG(to.uniprot(c3_genes), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")
c4_ <- enrichKEGG(to.uniprot(c4_genes), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")
c5_ <- enrichKEGG(to.uniprot(c5_genes), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")

### Plot!
Comparisons <- c(c1_, c2_, c3_, c4_, c5_)
names(Comparisons) <- c("cluster 1", "cluster 2", "cluster 3", "cluster 4", "cluster 5")
...
ORDER <- c("cluster 3", "cluster 4", "cluster 5", "cluster 2", "cluster 1")
###CREATE A FULL PATHWAYS MATRIX
dat <- data.frame(
  Pathway = rep(AllPaths,length(Comparisons)),
...
ggplot(data = dat, aes(x = Comparison, y = Pathway, size = GeneRatio, fill = FDR_cut05)) +
  ...
  coord_flip() +
  theme(axis.text.x = element_text(size = 6, angle = 270, hjust = 0, vjust = 0.5)) +
  ylab(NULL) + xlab(NULL)
```
...
```{r}
# Run pathways
cF_ <- enrichKEGG(to.uniprot(c(c1_genes, c2_genes)), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")
cA_ <- enrichKEGG(to.uniprot(c(c3_genes, c4_genes)), organism = "hsa", pvalueCutoff=1, keyType = "uniprot")

### Plot!
Comparisons <- c(cF_, cA_)
names(Comparisons) <- c("Fetal clusters (3,4)", "Adult clusters (1,2)")
# Gather all significant hits
AllPaths <- unique(unlist(sapply(
  Comparisons,
  function(X) X@result$Description[X@result$p.adjust<0.05])))
# Pick ORDER of Comparisons
ORDER <- c("Fetal clusters (3,4)", "Adult clusters (1,2)")
###CREATE A FULL PATHWAYS MATRIX
