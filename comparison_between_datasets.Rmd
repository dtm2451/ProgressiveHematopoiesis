...
sc4FvA <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-adult",
                     test.use = "MAST")
sc4FvA_padjFC <- sc4FvA[abs(sc4FvA$avg_logFC)>=log2(1.5) &
...
sc4FvC <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-cord",
                     test.use = "MAST")
sc4FvC_padjFC <- sc4FvC[abs(sc4FvC$avg_logFC)>=log2(1.5) &
...
sc4CvA <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-adult",
                     test.use = "MAST")
sc4CvA_padjFC <- sc4CvA[abs(sc4CvA$avg_logFC)>=log2(1.5) &
...
#Trim to DE cutoffs
FvA_padjFC <- FvA[(
   (FvA$padj<=0.05) & (abs(FvA$log2FoldChange)>=log2(1.5)) & (!is.na(FvA$padj))
   ),]
FvC_padjFC <- FvC[(
  (FvC$padj<=0.05) & (abs(FvC$log2FoldChange)>=log2(1.5)) & (!is.na(FvC$padj))
  ),]
CvA_padjFC <- CvA[(
  (CvA$padj<=0.05) & (abs(CvA$log2FoldChange)>=log2(1.5)) & (!is.na(CvA$padj))
  ),]
...
microarray_in <- microarray[is.gene(rownames(microarray),bulkCD4),]
...
bulkCD4@meta.data$age.names <- age.names

### Plot
# Create Heatmap
hm <- dittoHeatmap(
  genes = rownames(microarray_in), object = bulkCD4, annotation.metas = "Age",
  annotation_row = data.frame(
    'MicroarrayAnnotation' = enriched,
    row.names = rownames(microarray_in)),
  annotation.colors = c(dittoColors()[1:3], "red", "blue"),
  cutree_row = 2, cutree_col = 3,
  show.rownames = FALSE, cell.names.meta = "age.names",
...
Treg_UP_in <- Treg_UP[is.gene(Treg_UP, bulkCD4)]
...
Treg_DOWN_in <- Treg_DOWN[is.gene(Treg_DOWN, bulkCD4)] 

#Make age annotation metadata
bulkCD4@meta.data$Age.names <- paste(gsub("Cord", "UCB",meta("Age", bulkCD4)),1:5, sep = "-")
...
# highlights_in <- is.gene(highlights, bulkCD4, TRUE) # ALl are inside
...
all_exp_df <- data.frame(
  z.exp = 
    c(sapply(highlights, function(X) {
      mean(gene(X, bulkCD4, data.type = "relative")[meta("Age",bulkCD4)=="Fetal"])
    }),
    sapply(highlights, function(X) {
      mean(gene(X, bulkCD4, data.type = "relative")[meta("Age",bulkCD4)=="Cord"])
    }),
    sapply(highlights, function(X) {
      mean(gene(X, bulkCD4, data.type = "relative")[meta("Age",bulkCD4)=="Adult"])
    })),
...
hm <- dittoHeatmap(rownames(genes_annot),
    ...
    show.rownames = FALSE,
    annotation.colors = c(dittoColors()[1:3],"blue","red"))
