...
sc4FvA <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-adult",
                     test.use = "MAST")
sc4FvA_padjFC <- sc4FvA[abs(sc4FvA$avg_logFC)>=log(1.8, exp(1)) &
...
sc4FvC <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-cord",
                     test.use = "MAST")
sc4FvC_padjFC <- sc4FvC[abs(sc4FvC$avg_logFC)>=log(1.8, exp(1)) &
...
sc4CvA <- FindMarkers(Tcells,
                     ...
                     ident.2 = "4-adult",
                     test.use = "MAST")
sc4CvA_padjFC <- sc4CvA[abs(sc4CvA$avg_logFC)>=log(1.8, exp(1)) &
...
#Trim to DE cutoffs
FvA_padjFC <- FvA[(
   (FvA$padj<=0.05) & (abs(FvA$log2FoldChange)>=log2(1.8)) & (!is.na(FvA$padj))
   ),]
FvC_padjFC <- FvC[(
  (FvC$padj<=0.05) & (abs(FvC$log2FoldChange)>=log2(1.8)) & (!is.na(FvC$padj))
  ),]
CvA_padjFC <- CvA[(
  (CvA$padj<=0.05) & (abs(CvA$log2FoldChange)>=log2(1.8)) & (!is.na(CvA$padj))
  ),]
